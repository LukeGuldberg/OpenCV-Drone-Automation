...
#include <opencv2/calib3d/calib3d_c.h>      //CV_CALIB_FIX_PRINCIPAL_POINT
...
#include <opencv2/core/persistence.hpp>  //cvwritecomment
...
static void help() {
    cout << "This is a camera calibration sample." << endl
         ...
         << "Near the sample file you'll find the configuration file, which "
            "has detailed help of "
            "how to edit it.  It may be any OpenCV supported file format "
            "XML/YAML."
         << endl;
}
class Settings {
public:
    ...
    void write(FileStorage& fs) const  // Write serialization for this class
    {
        fs << "{"
           ...
           << "Calibrate_Pattern" << patternToUse << "Calibrate_NrOfFrameToUse"
           << nrFrames << "Calibrate_FixAspectRatio" << aspectRatio
...
public:
    Size boardSize;  // The size of the board -> Number of items by width and
                     // height
    Pattern calibrationPattern;  // One of the Chessboard, circles, or
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0;; ++i) {
        ...
        if (view.empty())  // If no more images then run calibration, save and
                           // stop loop.
        {
        ...
        bool found;
        switch (
            s.calibrationPattern)  // Find feature points on the input format
        {
        ...
        if (found)  // If done with success,
        {
            ...
            if (mode == CAPTURING &&  // For camera only take new samples after
                                      // delay time
        ...
        if (mode == CAPTURING) {
            if (s.showUndistorsed)
                msg =
                    format("%d/%d Undist", (int)imagePoints.size(), s.nrFrames);
            else
